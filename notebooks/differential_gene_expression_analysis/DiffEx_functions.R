...
get_GO_analysis_plots <- function(cell_type, ct_print, cont_tmp, tt){
  # TODO: maybe change filtering?
  tt_signif <- tt[tt$logFC >= 2 | tt$logFC <= -2, ]
  tt_signif <- arrange(tt_signif, desc(abs(logFC)))
  tt_signif <- head(tt_signif, 250)
  ...
  ggsave(
    paste('Dotplot_padj_', ct,'_', cont_tmp, '.png', sep = ""),
  ...
  # heat plot
  heatplot(ego, foldChange=gene_list, showCategory=10) +
    ggtitle(paste("Heat Plot for ", ct_print, " (", cont_tmp, ")", sep = "")) +
    theme(plot.title = element_text(size = 15, face = "bold")) +
    ggplot2::coord_flip()
  ggsave(
    paste('Heatplot_padj_', ct,'_', cont_tmp, '.png', sep = ""),
    plot = last_plot(), device = NULL, path = path_out, scale = 1, width = 20,
  ...
  p2 <- heatplot(ego, foldChange=gene_list, showCategory=5)
  cowplot::plot_grid(p1, p2, ncol=1, labels=LETTERS[1:2])
  ggsave(
    paste('Heatplot_classi_padj_', ct,'_', cont_tmp, '.png', sep = ""),
