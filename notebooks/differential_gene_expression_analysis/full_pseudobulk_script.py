...
adata = sc.read("/lustre/groups/ml01/workspace/daniel.michaela.masterpraktikum23/all_datasets_merged/complete_merged_public_and_galapagos_integrated_scvi_neighbors_umap_withfinalanno.h5ad")
...
adata.obs['condition_2'] = adata.obs.loc[:, 'condition']

adata.obs['condition_2'] = adata.obs['condition_2'].replace('saline','untreated')

adata.obs['condition_2'].unique()

adata = adata[adata.obs.dataset != "schiller"]

print("removed schiller")
...
plt.savefig("/lustre/groups/ml01/workspace/daniel.michaela.masterpraktikum23/masterpraktikum_fibrosis_atlas/notebooks/differential_gene_expression_analysis/outputfigs/raw_counts_histogram.png", dpi = 600)
...
adata.obs["sample"] = [
    ...
    f"{rep}_{l}" for rep, l in zip(adata.obs["batch"], adata.obs["condition_2"])
...
adata.obs['condition_2'] = adata.obs["condition_2"].astype(str)
...
# Iterate through the values in the specified obs column and replace "-" with "_"

obs_df["final_annotation"] = obs_df["final_annotation"].str.replace("-", "_")

obs_df["final_annotation"] = obs_df["final_annotation"].str.replace(" ", "_")

#obs_df["final_annotation"] = obs_df["final_annotation"].str.replace("+", "plus")

obs_df["final_annotation"] = obs_df["final_annotation"].str.replace("/", "and_") #careful, there are some cells with - and + in their names, check the minus (-) ones and rename them to "minus", see line below

obs_df["final_annotation"] = obs_df["final_annotation"].str.replace("4_and_Cd8__T", "4_minus_Cd8_minus_T")

obs_df["final_annotation"] = obs_df["final_annotation"].astype('category')


# Update the AnnData object's observations with the modified DataFrame

adata.obs = obs_df

print("changed the annotation names, see printout if it worked")
